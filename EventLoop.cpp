...
void EventLoop::runAt(struct timespec expire, TimerCallback timerCallback)
{
	struct timespec zeroInterval;
	memset(&zeroInterval, 0, sizeof(struct timespec));
	Timer timer(expire, zeroInterval, timerCallback);
	timerQueue_->addTimer(timer);
}

void EventLoop::runAfter(struct timespec interval, TimerCallback timerCallback)
{
...
void EventLoop::runEvery(struct timespec interval, TimerCallback timerCallback)
{
	struct timespec expire;
	memset(&expire, 0, sizeof(struct timespec));
	clock_gettime(CLOCK_REALTIME, &expire);
	...
	Timer timer(expire, interval, timerCallback);
	timerQueue_->addTimer(timer);
}
